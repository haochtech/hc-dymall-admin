...
class LoginForm extends ApiModel
{
    ...
    public function loginTT()
    {
        $config = MpConfig::get($this->store->id);
        $tt_app_id = $config['tt_app_id'];
        $tt_app_secret = $config['tt_app_secret'];
        if (!$tt_app_id || !$tt_app_secret){
            return new ApiResponse(1,'服务器配置错误');
        }
        $api = "https://developer.toutiao.com/api/apps/jscode2session?appid={$tt_app_id}&secret={$tt_app_secret}&code={$this->code}&anonymous_code={$this->anonymous_code}";
        $curl = new Curl();
