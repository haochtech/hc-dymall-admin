        ...
        if ($this->order_id) { //单个订单付款
            ...
            if ($this->pay_type == 'WECHAT_PAY') {
                ...
                if (\Yii::$app->fromTouTiaoApp()) {
                    return $this->ttUnifiedOrder($goods_names);
                }
...
//                'alipay_url' => ''
    ...
    public function WechatH5Pay($out_trade_no, $total_fee, $notifyUrl, $body, $web_url, $redirect_url)
    {
        $config = MpConfig::get($this->store->id);
        $paydata = array(
            'appid' => $config['wechat_app_id'],//"wxc7811d73da68c79e",
            'mch_id' => $config['wechat_mch_id'],//"1491703102",
...
//        $str = http_build_query($data) . "&key=" . 'fnR8NbPlqZ9DtGQteyRwcFRsjMcpoek4';//$this->key;
        $str = http_build_query($data) . "&key=" . $config['wechat_mch_secret'];
