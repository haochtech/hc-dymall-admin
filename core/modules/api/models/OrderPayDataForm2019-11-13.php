...
class OrderPayDataForm extends ApiModel
{
    ...
    private function dyUnifiedOrder($goods_names){
        ...
        $string = $var . 's4eb6exfdni1kdmks46hqb9sjjki5wbmkew4z3c2';
...
//            ];
        ...
        if ($this->pay_way == 'wechat') {
            ...
            $arr = [
                ...
                'out_order_no' => $this->order->order_no,
                'currency' => 'CNY',
                'subject' => '懂车帝xx 活动测试',//$goods_names
                'body' => $goods_names,
                'trade_time' => time(),
                'valid_time' => 300,
                'notify_url' => pay_notify_url('/pay-notify.php'),
...
//                'wx_type' => 'MWEB',
    ...
    public function WechatH5Pay($out_trade_no, $total_fee, $notifyUrl, $body, $web_url, $redirect_url)
    {
        $config = MpConfig::get($this->store->id);
        $paydata = array(
            'appid' => 'wx72c26c4b23b771b0',//"wxc7811d73da68c79e",  $config['wechat_app_id']
            'mch_id' => '1513892911',//"1491703102", $config['wechat_mch_id']
...
//        $str = http_build_query($data) . "&key=" . 'fnR8NbPlqZ9DtGQteyRwcFRsjMcpoek4';//$this->key;
        $str = http_build_query($data) . "&key=" . "Sekahuizhinengmendian11111111111"; //$config['wechat_mch_secret']
